<?php
/*
Plugin Name: Pito Secure Invites
...
function secure_invite_default_setting($name) {
	...
	if ($name == "secure_invite_no_invite_message") { return "Sorry, you must be invited to join this community."; }
...
function secure_invite_send()
{
	...
	if (secure_invite_user_can_invite( $current_user->ID ))
	{
		...
		(%d, %s, now());", $current_user->ID, $to);
			...
			{
				if(!empty($pname)) {
					$subject = 'invitation to join secret sessions';
					$message = "Dear ".$pname.", ";
				}
				else {
					$subject = 'invitation to join secret sessions';
