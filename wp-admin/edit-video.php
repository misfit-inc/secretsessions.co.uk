...
$screen->set_help_sidebar(
	...
	if(!isset($_GET['vid'])){
		wp_redirect(get_admin_url().'secret-sessions.php'); exit; 
	}else{
		$vid				=	$_GET['vid'];
		$video_data_query	=	"SELECT * from videos where id=$vid";
		$video_data			=	$wpdb->get_row ($video_data_query);
		if($video_data){
			$video_title	=	$video_data->video_title;
			$video_url		=	$video_data->video_link;
			$video_image	=	$video_data->video_image;
			$ss_artist_name	=	$video_data->ss_artist_name;
	...
	if(isset($_POST['youtube'])){
		...
		if( !empty( $_FILES['videoImage']['name']))	{	
			...
			$data_array['video_image']		=	$video_image['url'];
			$data_array['image_path']		=	$video_image['file'];
		}
			$table = "videos";
			$data_array['video_link']		=	$link;
			$data_array['video_title']		=	$title;
			$data_array['ss_artist_name']	=	$ss_artist_name;
			$data_array['user_id']	=	$user_id_select;
			$where = array('id' => $vid);
...
<form action="" method="post" name="createuser" id="createuser" class="validate" novalidate="novalidate" enctype="multipart/form-data">
...
<table class="form-table">
    ...
    <tr class="form-field form-required">
		<th scope="row"><label for="user_login">Video Tiltle <span class="description">(required)</span></label></th>
		<td><input name="title" type="text" id="user_login" value="<?php if(isset($_POST['title'])){ echo $_POST['title'];}else{ echo $video_title;}?>" aria-required="true"></td>
	</tr>
    <tr class="form-field">
		<th scope="row"><label for="user_login">Artist name <span class="description">(required)</span></label></th>
		<td><input name="artist_name" type="text" id="user_login" value="<?php if(isset($_POST['artist_name'])){ echo $_POST['artist_name'];}else{ echo $ss_artist_name;}?>"></td>
	</tr>
	<tr class="form-field form-required">
		<th scope="row"><label for="email">Youtube link <span class="description">(required)</span></label></th>
		<td><input name="youtube" type="text" id="email" value="<?php if(isset($_POST['youtube'])){ echo $_POST['youtube'];}else{ echo $video_url;}?>"></td>
	</tr>
                <tr class="form-field form-required">
                            ...
                            <?php
                                if ( count( $users_ids ) > 0 ) {
                                    foreach ( $users_ids as $id ) {
                                        ?>
                                        <option
                                        value="<?php echo $id ?>" <?php if($id==$user_id) echo 'selected="selected"';?>  ><?php echo character_limiter( get_cimyFieldValue( $id , 'NAME' ) , 15 ); ?></option><?php
                                    }
                                }
                            ?>
                        </select>
	...
	<tr class="form-field">
		...
		<td><img width="20%" src="<?php echo $video_image?>" /></td>
	</tr>
	</tbody></table>


<p class="submit"><input type="submit" name="createuser" id="createusersub" class="button button-primary" value="Update video"></p>
</form>
</div>
<?php
