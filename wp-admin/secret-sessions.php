...
// Help tabs
    ...
    $videos_count = $wpdb->get_results( "SELECT count(*) as total from videos where is_admin_added=1" );
    ...
    $ss_feature_videos = $wpdb->get_row( "SELECT count(*) as total from videos where ss_featured=1 and user_id=0" );
    $ss_feature_count = $ss_feature_videos->total;
    ...
    $like_sort_order = 'asc';
    $like_url = admin_url( 'secret-sessions.php?orderby=like&order=desc' );
    if ( isset( $_GET[ 'orderby' ] ) && ( $_GET[ 'orderby' ] == 'like' ) ) {
        ...
        if ( isset( $_GET[ 'order' ] ) && ( $_GET[ 'order' ] == 'desc' ) ) {
            $like_url = admin_url( 'secret-sessions.php?orderby=like&order=asc' );
            $like_sort_order = 'desc';
        }
    }
    $featured_sorted = 'sortable';
    $featured_sort_order = 'asc';
    $featured_url = admin_url( 'secret-sessions.php?orderby=featured&order=desc' );
    if ( isset( $_GET[ 'orderby' ] ) && ( $_GET[ 'orderby' ] == 'featured' ) ) {
        ...
        if ( isset( $_GET[ 'order' ] ) && ( $_GET[ 'order' ] == 'desc' ) ) {
            $featured_url = admin_url( 'secret-sessions.php?orderby=featured&order=asc' );
            $featured_sort_order = 'desc';
        }
    }

    $ss_featured_sorted = 'sortable';
    $ss_featured_sort_order = 'asc';
    $ss_featured_url = admin_url( 'secret-sessions.php?orderby=ssfeatured&order=desc' );
    if ( isset( $_GET[ 'orderby' ] ) && ( $_GET[ 'orderby' ] == 'ssfeatured' ) ) {
        $ss_featured_sorted = 'sorted';
        if ( isset( $_GET[ 'order' ] ) && ( $_GET[ 'order' ] == 'asc' ) ) {
            $ss_featured_url = admin_url( 'secret-sessions.php?orderby=ssfeatured&order=desc' );
            $ss_featured_sort_order = 'asc';
        }
        if ( isset( $_GET[ 'order' ] ) && ( $_GET[ 'order' ] == 'desc' ) ) {
            $ss_featured_url = admin_url( 'secret-sessions.php?orderby=ssfeatured&order=asc' );
            $ss_featured_sort_order = 'desc';
        }
    }
    ...
    $order_by_field = 'id';
    $order = 'desc';
    $url = admin_url( 'secret-sessions.php' );
    if ( isset( $_GET[ 'orderby' ] ) ) {
        ...
        if ( $_GET[ 'orderby' ] == 'ssfeatured' ) {
            $order_by_field = 'ss_featured';
            $url .= '?orderby=ssfeatured';
        }
        if ( $_GET[ 'order' ] == 'asc' ) {
            $url .= '&order=asc';
            $order = 'asc';
        ...
        $url = admin_url( 'secret-sessions.php?orderby=id&order=desc' );
    }
	
	if(isset($_GET['pager'])) {
		if($_GET['pager']=='all'){
			$id		=	1;
			$sql	=	"SELECT  videos.*,(select count(*) from video_like where video_like.video_id = videos.id)  as likes FROM videos where videos.is_admin_added=1 order by $order_by_field $order";
		}else{
			$id		=	$_GET['pager'];
			$start	=	($id-1)*$limit;
			$sql	=	"SELECT  videos.*,(select count(*) from video_like where video_like.video_id = videos.id)  as likes FROM videos where videos.is_admin_added=1 order by $order_by_field $order limit $start, $limit";
		}
	}else{
		$id=1;
			$sql	=	"SELECT  videos.*,(select count(*) from video_like where video_like.video_id = videos.id)  as likes FROM videos where videos.is_admin_added=1 order by $order_by_field $order limit $start, $limit";
...
?>
    <div class="wrap"><h2>
            Secret Sessions Videos <a href="add-video.php" class="add-new-h2">Add New video</a>
    </h2>
<p><a href="<?php echo $url?>&pager=all">Show All videos</a></p>
<P style="text-align:center; color:#093;"><?php if(isset($_SESSION['msg'])) { echo $_SESSION['msg']; unset($_SESSION['msg']);}  ?></P>
        <table class="wp-list-table widefat fixed posts">
            <thead>
            <tr>
                <th style="" class="manage-column" id="" scope="col">Title</th>
                <th style="" class="manage-column" id="" scope="col">Artist</th>
                <th style="" class="manage-column" id="" scope="col">Link</th>
                <th style="" class="manage-column" id="" scope="col">Image</th>

                <th style=""
                    class="manage-column column-share <?php echo $featured_sorted ?> <?php echo $featured_sort_order ?>"
                    id="share" scope="col">
                ...
                <th style=""
                    class="manage-column column-share <?php echo $ss_featured_sorted ?> <?php echo $ss_featured_sort_order ?>"
                    id="share" scope="col">
                    <a href="<?php echo $ss_featured_url ?>">
                        <span>Secret sessions Featured</span>
                        <span class="sorting-indicator"></span>
                    </a>
                </th>

                <th style=""
                    class="manage-column column-share <?php echo $like_sorted ?> <?php echo $like_sort_order ?>"
                    id="share" scope="col">
...
?>
            ...
            <?php foreach ( $videos as $vid ) { ?>
                <tr id="post-53"
                    class="post-53 type-sescret_session_home status-publish hentry alternate iedit author-self level-0">
                    <td class="post-title page-title column-title"><strong><?php echo stripslashes($vid->video_title) ?></strong>

                        <div class="row-actions">
                    <span class="inline hide-if-no-js">
                        <a href="<?php echo get_admin_url() . 'edit-video.php?vid=' . $vid->id . '&pager=' . $id ?>"
                           title="Delete video">Edit</a>|
                    </span>
                    <span class="inline hide-if-no-js">
                        <a href="<?php echo get_admin_url() . 'secret-session-video.php?action=delete&vid=' . $vid->id . '&pager=' . $id ?>"
                           title="Delete video">Delete</a>|
                    </span>
                    <span class="inline hide-if-no-js">
                    	<?php if ( $ss_feature_count == 0 ) { ?>
                            <a href="<?php echo get_admin_url() . 'secret-session-video.php?action=ssfeature&vid=' . $vid->id . '&pager=' . $id ?>"
                               title="Delete video">Secretsession featured</a>|
                        <?php
                        } else {
                            if ( $vid->ss_featured == 1 ) {
                                ?>
                                <a href="<?php echo get_admin_url() . 'secret-session-video.php?action=rssfeature&vid=' . $vid->id . '&pager=' . $id ?>"
                                   title="Delete video">remove secretsession featured</a>|
                            <?php
                            }
                        } ?>
                    </span>
                            <?php if ( $vid->is_featured == '1' ) { ?>
                                <span>
                            <a class="submitdelete" title="Remove Featured"
                               href="<?php echo get_admin_url() . 'secret-session-video.php?action=rfeature&vid=' . $vid->id . '&pager=' . $id ?>">Remove
                                home featured</a>|
                        </span>
                            <?php } elseif ( $feature_count < 18 ) { ?>
                                <span>
                            <a class="submitdelete" title="Home Featured"
                               href="<?php echo get_admin_url() . 'secret-session-video.php?action=feature&vid=' . $vid->id . '&pager=' . $id ?>">Home
                                featured</a>|
                        </span>
                            <?php } else { ?>
                                <span style="color:red;">
                            18 videos allowed in featured
                        </span>
                            <?php } ?>
                        </div>
                    </td>
					...
					if($vid->user_id==0){
						echo character_limiter(stripslashes($vid->ss_artist_name)); 
					}else{
						echo character_limiter( get_cimyFieldValue( $vid->user_id , 'NAME' ) , 15 );
					}
					?></td>
                    <td class="date column-date"><?php echo $vid->video_link ?></td>
                    <td class="date column-date"><img style="width:60%;" src="<?php echo $vid->video_image ?>" alt=""
